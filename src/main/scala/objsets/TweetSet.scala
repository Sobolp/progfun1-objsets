...
abstract class TweetSet extends TweetSetInterface :
  ...
  def descendingByRetweet: TweetList =
    def processSet(set: TweetSet, acc: TweetList): TweetList =
      val currMax = set.mostRetweeted
      if currMax.retweets == 0
      then acc
      else Cons(currMax, processSet(set.remove(currMax), acc))
  ...
  def mostRetweeted =
    var maxRetweeted = Tweet("", "", 0)
    foreach(t => if t.retweets > maxRetweeted.retweets then maxRetweeted = t)
    maxRetweeted
